...
for(i in 1:length(samples)) {
        models[[i]] <- lm(aver.resor.prot ~ timer, data=calc2[calc2$smpl==samples[i], ])
        legendary <- paste0(legendary, 
                            "\n",
                            samples[i],
                            ": ",
                            get.formula(models[[i]])
        )
}

print(
        ggplot(aes(timer, aver.resor.prot, color=smpl), data=calc2) +
                geom_point() +
                annotate("text",
                         x=min(calc2$timer)+point.two(calc2$timer),
                         y=max(calc2$aver.resor.prot)-point.two(calc2$aver.resor.prot),
                         label=legendary
                )
